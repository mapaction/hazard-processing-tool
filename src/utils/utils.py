...
def compute_population_exposure(
    hazard_mask_raster: xr.DataArray, population_raster: xr.DataArray
) -> xr.DataArray:
    """ "
...
def prep_data() -> Dict[str, xr.DataArray]:
    ...
    for hazard in HAZARD_RASTER_PATH:
        ...
        hazard_mask_raster = compute_hazard_mask(
            hazard_raster, population_raster, HAZARD_THRESHOLD[hazard]
        )
        population_exposure_raster = compute_population_exposure(
            hazard_mask_raster, population_raster
        )
...
def compute_hazard_population_exposure(
    ...
    exposure_df = admin_df.drop(columns="geometry").copy()
    exposure_df["pop_exp"] = compute_zonal_stat(
        exp_arr, exp_affine, admin_df, agg="sum"
    )
    exposure_df["pop_tot"] = compute_zonal_stat(
        pop_arr, pop_affine, admin_df, agg="sum"
    )
    exposure_df["exp_ratio"] = exposure_df["pop_exp"] / exposure_df["pop_tot"]
    return exposure_df


def compute_binary_zonal_stat(
    raster_path: str, admin_df: gpd.GeoDataFrame, threshold: float = 0
) -> List[float]:
    """
